...
import { ParsedGitHubUrl, GitHubTreeResponse } from './types';
...
async function getBranchSha(
  ...
  if (!sha) {
    throw new Error('Failed to parse branch SHA from GitHub response');
  }
  return sha;
}
...
async function getCommitTreeSha(
  ...
  if (!sha) {
    throw new Error('Failed to parse commit tree SHA from GitHub response');
  }
  return sha;
}
...
async function downloadBlob(
  ...
  if (!data.content) {
    throw new Error('Failed to parse blob content from GitHub response');
  }
  ...
  for (const item of relevantItems) {
    if (item.type === 'blob') {
      ...
      if (!targetPath.startsWith(resolvedTargetDir)) {
        throw new Error(`Refusing to write file outside target directory: ${item.path}`);
      }
